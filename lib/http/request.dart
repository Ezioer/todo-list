...
import 'package:todos/base/get/get_extension.dart';
import 'package:todos/http/http_request.dart';
import 'package:todos/http/request_api.dart';
...
class Request {
  ...
  static void oldGet<T>(
    ...
    _oldRequest(Method.GET, url, parameters,
        isJson: isJson, dialog: dialog, success: success, fail: fail);
  }
    ...
    HttpRequest.request(method, url, parameters, isJson: isJson,
      ...
      if (success != null) {
          ...
          if (resultModel.status == "success") {
            ...
            ///其他状态，弹出错误提示信息
            ToastUtils.show(resultModel.status);
          }
      }
    }, fail: (code, msg) {
  ...
  static void _oldRequest<T>(
    ...
    HttpRequest.request(method, url, parameters, isJson: isJson,
      ...
      if (success != null) {
        // var resultModel = OldPoetryEntity.fromJson(result);
        // debugPrint("request success =>$resultModel");
        success(result);
      }
    }, fail: (code, msg) {
